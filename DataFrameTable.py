...
class DataFrameTable(ttk.Frame):
    ...
    """
    def __init__(self, master: Any, engine: Optional[Any] = None, db_type: str = 'PostgreSQL',
                 ...
                 table_name: Optional[str] = None, on_data_change: Optional[Callable[[pd.DataFrame], None]] = None, **kwargs):
        ...
        try:
            ...
            self.treeview_frame = TreeViewFrame(
                master=self, show_edit_modal=self.show_edit_modal, df=self.df,
                column_width=self.column_width, log_message=log_message
            )
    ...
    def show_edit_modal(self, index=None):
        ...
        try:
            ...
            self.modal_edit = EditModal(
                master=self, engine=self.engine, df=self.df, row_index=self.selected_row_index,
                name_campo_primary_key=campo_primary_key, primary_key_value=primary_key_value,
                table_name=self.table_name, on_data_change=self.on_data_change, edit_enabled=self.edit_enabled
            )

        except Exception as e:
            log_message(self, f"Erro ao abrir modal de edição: {e} ({type(e).__name__})\n{traceback.format_exc()}", level="error")
